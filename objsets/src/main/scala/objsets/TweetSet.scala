...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
   if(!that.contains(this.elem)){
     right.union(left.union(that.incl(this.elem)));
   }else{
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter((p: Tweet)=>google.exists((str:String)=>p.text.contains(str)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter((p: Tweet)=>apple.exists((str:String)=>p.text.contains(str)))
